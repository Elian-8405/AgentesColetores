...
class AgenteReativoSimples(Agente):
    ...
    def mover(self):
        ...
        movimentos_validos = [
            ...
            and self.grid.grid[x][y] in [0, 10, 20, 50]  # Permite movimento para células vazias ou com recursos
        ]
...
class AgenteBaseadoEstado(Agente):
    ...
    def mover(self):
        ...
        if movimentos_validos:
            nova_pos = random.choice(movimentos_validos)
            self.grid.atualizarPosicaoAgente(self.posicaoAtual, nova_pos)
            self.posicaoAtual = nova_pos
    ...
    def coletar(self):
        ...
        elif recurso == 50:  # Coleta Estruturas Antigas
            self.grid.grid[x][y] = 0
            print(f"AgenteBaseadoEstado coletou Estrutura Antiga em ({x}, {y}).")
...
class AgenteBaseadoObjetivos(Agente):
    ...
    def mover(self):
        ...
        if self.objetivo:
            ...
            if self.grid.grid[movimento[0]][movimento[1]] == 0:
                self.grid.atualizarPosicaoAgente(self.posicaoAtual, movimento)
    ...
    def coletar(self):
        ...
        if recurso == 10:  # Coleta Cristais Energéticos
            ...
            self.grid.grid[x][y] = 0
            print(f"AgenteBaseadoObjetivos coletou Estrutura Antiga em ({x}, {y}).")
